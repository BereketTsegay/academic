<template>
    <div>
       <div v-if="message" :class="successClass" role="alert">
       ...
       <DataTable  title="Vessels"
            :isLoading="isLoading"
            @paginate="($event)=>getData(null,$event)"
            :pagination="paginationdata"
            @search="($event)=>getData($event,1)"
            :data="vessels" @add="add()" @refresh="getData()" :columns="columns" @edit="($event)=>edit($event)" @delete="($event)=>destroy($event)" @view="($event)=>view($event)"></DataTable>
       <BnModal v-if="!!createToggle" :toggle="createToggle" @close="createToggle= !createToggle" title="Vessel Form">
            <vesselForm :vessel="selectedData" @updateData="($event)=>updated($event)"></vesselForm>
       </BnModal>
...
export default {
    components:{ DataTable, BnModal, vesselForm, Confrim,brandView },
    data(){
        return{
            vessels:[],
    ...
    methods: {
        getData(query='',page=1){
            ...
            this.isLoading=true;
            var request_str = '/api/auth/vessels?page='+page;
        ...
        updated(rec){
            console.log(rec);
            var found =this.vessels.findIndex((element) => element.id == rec.id);
            !!found ? this.vessels[found]=rec : this.vessels.push(rec);
            this.selectedData={},
        ...
        deleteData(){
            axios.delete('/api/auth/vessel/'+this.selectedData.id)
            .then((res)=>{
                this.message=res.data.payload;
                var found =this.vessels.findIndex((element) => element.id == this.selectedData.id);
               if(found){
                this.vessels.splice(found,1);
                this.selectedData={};
               }
