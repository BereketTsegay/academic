<template>
    <div>
       ...
       <DataTable  title="Unit" :isLoading="isLoading" @paginate="($event)=>getData(null,$event)" :pagination="paginationdata" @search="($event)=>getData($event,1)" :data="units" @add="add()" @refresh="getData()" :columns="columns" @edit="($event)=>edit($event)" @delete="($event)=>destroy($event)" @view="($event)=>view($event)"></DataTable>
       <BnModal v-if="!!createToggle" :toggle="createToggle" @close="createToggle= !createToggle" title="Category Form">
            <unitForm :unit="selectedData" @saved="($event)=>updated($event)"></unitForm>
       </BnModal>
...
import unitForm from './category/unitForm.vue';
import brandView from '../../../components/brandView.vue';

export default {
    components:{ DataTable, BnModal, unitForm, Confrim,brandView },
    data(){
        return{
            units:[],
            isLoading:false,
    ...
    methods: {
        getData(query='',page=1){
            ...
            this.isLoading=true;
            var request_str = '/api/auth/units?page='+page;
        ...
        updated(rec){
            var found =this.units.findIndex((element) => element.id == rec.id);
            found > -1 ? this.units[found]=Object.assign({},rec) : this.units.push(rec);
            this.selectedData={},
        ...
        deleteData(){
            axios.delete('/api/auth/unit/'+this.selectedData.id)
            .then((res)=>{
                this.message=res.data.payload;
                var found =this.units.findIndex((element) => element.id == this.selectedData.id);
               if(found){
                this.units.splice(found,1);
                this.selectedData={};
               }
