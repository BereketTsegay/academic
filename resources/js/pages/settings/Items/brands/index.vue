<template>
    <div>
       ...
       <BnModal v-if="!!createToggle" :toggle="createToggle" @close="createToggle= !createToggle" title="Category Form">
            <unitForm :brand="selectedData" @saved="($event)=>{getData();createToggle=false}"></unitForm>
       </BnModal>
...
import unitForm from './brandForm.vue';
...
export default {
    ...
    methods: {
        getData(query='',page=1){
            ...
            this.isLoading=true;
            var request_str = '/api/auth/brands?page='+page;
        ...
        updated(rec){
            var found =this.brands.findIndex((element) => element.id == rec.id);
            found > -1 ? this.brands[found]=Object.assign({},rec) : this.brands.push(rec);
            this.selectedData={},
        ...
        deleteData(){
            axios.delete('/api/auth/brand/'+this.selectedData.id)
            .then((res)=>{
                this.message=res.data.payload;
            //     var found =this.brands.findIndex((element) => element.id == this.selectedData.id);
            //    if(found){
            //     this.brands.splice(found,1);
            //     this.selectedData={};
            //    }
            this.getData();
