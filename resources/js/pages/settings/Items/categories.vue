<template>
    <div>
       ...
       <DataTable title="Category" :isLoading="isLoading" @paginate="($event)=>getData(null,$event)" :pagination="paginationdata" @search="($event)=>getData($event,1)" :data="categories" @add="add()" @refresh="getData()" :columns="columns" @edit="($event)=>edit($event)" @delete="($event)=>destroy($event)" @view="($event)=>view($event)"></DataTable>
       <BnModal v-if="!!createToggle" :toggle="createToggle" @close="createToggle= !createToggle" title="Category Form">
            <categoryForm :category="selectedData" @saved="($event)=>updated($event)"></categoryForm>
       </BnModal>
...
export default {
    components:{ DataTable, BnModal, categoryForm, Confrim,brandView},
    data(){
        return{
            categories:[],
            isLoading:false,
    ...
    methods: {
        getData(query='',page=1){
            ...
            this.isLoading=true;
            var request_str = '/api/auth/categories?page='+page;
        ...
        updated(rec){
            var found =this.categories.findIndex((element) => element.id == rec.id);
         
            found > -1 ? this.categories[found]=Object.assign({},rec) : this.categories.push(rec);
            this.selectedData={},
        ...
        deleteData(){
            axios.delete('/api/auth/category/'+this.selectedData.id)
            .then((res)=>{
                this.message=res.data.payload;
                var found =this.categories.findIndex((element) => element.id == this.selectedData.id);
               if(found > -1){
